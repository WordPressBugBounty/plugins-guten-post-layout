        ...
        public function content( $attributes, $has_ajax = false ){
           ...
           $paged = isset($attributes['paged']) ? $attributes['paged'] : 1;
            ...
            $args = array(
                ...
                'order'       => sanitize_sql_orderby($attributes['order']),
                'orderby'     => sanitize_sql_orderby($attributes['orderBy']),
                'ignore_sticky_posts' => 1,
                'posts_per_page' => intval($attributes['postscount']),
                'paged' => $paged,
            );
            ...
            if( isset($attributes['postOffset']) && $attributes['postOffset'] && ! ($args['paged'] > 1 ) ){
                $args['offset'] = isset($attributes['postOffset']) ? intval($attributes['postOffset']) : 0;
            }

            $taxonomyName = isset($attributes['taxonomyName']) ? $attributes['taxonomyName'] : '';

			$categories = isset( $attributes['categories'] ) && !empty($attributes['categories']) ? sanitize_text_field($attributes['categories']) : 'all';

			if( is_array($categories) ){
                $args['tax_query'] = array(
					array(
						'taxonomy' => sanitize_text_field($taxonomyName),
						'field' => 'term_id',
                ...
                if (isset($attributes['queryCat'])) {
                    if ($attributes['queryTaxonomy'] == 'categories' && !empty($attributes['queryCat'])) {
                        if( $attributes['queryCat'] != '[]' ) {
                            $args['tax_query'] = array(array('taxonomy' => 'category', 'field' => 'slug', 'terms' => sanitize_text_field($attributes['queryCat'])));
                        }
                    }
                }
                ...
                if (isset($attributes['queryTag'])) {
                    ...
                    if ($attributes['queryTaxonomy'] == 'tags' && !empty($attributes['queryTag'])) {
                        if( $attributes['queryTag'] != '[]' ) {
                            $args['tax_query'] = array(array('taxonomy' => 'post_tag', 'field' => 'slug', 'terms' => sanitize_text_field($attributes['queryTag'])));
                        }
                    }
                }
            }
            ...
            $widthClass = (isset($attributes['postBlockWidth']) && $attributes['postBlockWidth'] ) ? 'align'.esc_attr($attributes['postBlockWidth']) : '';
            ...
            if( $attributes['postLayout'] === 'slides'){
                ...
                $random_number = wp_rand(10, 1000);
                $markup .= sprintf('<div id="gpl-slick-slider-'.esc_attr($random_number).'" class="gpl-slick-slider '.esc_attr($widthClass).'" data-count="%1$d" data-slides-to-show="%2$s" data-autoplay="%3$s" data-navigation="%4$s">', count( $recent_posts ), esc_attr($attributes['slidesToShow']), esc_attr($attributes['autoPlay']), esc_attr($attributes['navigation']));
                ...
                if( isset($attributes['displayPostHeading']) && $attributes['displayPostHeading'] && isset($attributes['postHeadingText']) && $attributes['postHeadingText'] ) {
                    ...
                    $sub_heading = isset($attributes['displayPostSubHeading']) && $attributes['displayPostSubHeading'] ? '<p>'.esc_html($attributes['postSubHeadingText']).'</p>' : '';
				...
				'.$sub_heading.'
                        ...
                        esc_attr($attributes['postHeadingLinkTarget']),
                        esc_html( $attributes['postHeadingText'] ),
                        esc_attr($attributes['postHeadingAlign']),
                        esc_attr($attributes['postHeadingStyle'])
                    );
                    $markup .= '</div>';
                }

                $markup .= sprintf('<div  class="gpl-post-slider-one wp-block-guten-post-layout-post-grid post-grid-view gpl-d-flex gpl-flex-wrap %1$s %2$s" data-layout="%1$s" style="--item-padding-left-right : '.esc_attr($attributes['columnGap']).'px; --item-margin-bottom : '.(esc_attr($attributes['columnGap'])*2).'px; --item-height : '.(300-esc_attr($attributes['columnGap'])).'px; --image-height:'.(esc_attr($attributes['imageHeight'])).'px;" >', esc_attr($attributes['carouselLayoutStyle']), esc_attr($attributes['additionalCssClasses']));

                foreach ( $recent_posts as $post ){
                    $post_id = $post['ID'];
                    $post_thumbnail_id = get_post_thumbnail_id($post_id);

                    $gridView = $attributes['postLayout'] === 'grid' ? 'post-item gpl-mb-30 gpl-column-'.esc_attr($attributes['columns']).'' : 'post-item gpl-mb-30';

                    $image_src = $post_thumbnail_id ? wp_get_attachment_image_src( $post_thumbnail_id, '' . esc_attr($attributes['postImageSizes']) . '', false)[0] : '';
                    ...
                    $markup .= sprintf( '<article class="%1$s %2$s">', esc_attr($gridView), esc_attr($attributes['carouselLayoutStyle']) );
                    // start the post content wrap
                    $markup .= '<div class="post-content-area ' . esc_attr($attributes['align']) . ' '.$hasImage.'">';
            ...
            if( $attributes['postLayout'] !== 'slides') {
                $columnGap = isset($attributes['columnGap']) ? esc_attr($attributes['columnGap']) : '';
                $imageHeight = isset($attributes['imageHeight']) && is_numeric($attributes['imageHeight']) ? esc_attr($attributes['imageHeight']).'px' : null;
                $itemHeight = (int)$imageHeight && (int)$columnGap ? ((int)$imageHeight/2) - $columnGap .'px' : '285px';

                $firstPostItem = count($recent_posts) > 0 && esc_attr($attributes['gridLayoutStyle']) === 'g_skin2' ? array_splice($recent_posts, 0, 1) : '';
                ...
                $markup_before .= sprintf( '<div class="%1$s %2$s %3$s %4$s %5$s" style="--item-padding-left-right : '.esc_attr($attributes['columnGap']).'px; --item-minus-padding-left-right : -'.(esc_attr($attributes['columnGap'])).'px; --item-margin-bottom : '.(esc_attr($attributes['columnGap'])*2).'px; --item-height: '.$itemHeight.'; --image-height:'.(esc_attr($attributes['imageHeight'])).';">', esc_attr( $gridViewWrapper ), esc_attr($attributes['gridLayoutStyle']), $widthClass, $_gpl_row, esc_attr($attributes['additionalCssClasses']) );
                ...
                if( (isset($attributes['displayPostHeading']) && $attributes['displayPostHeading']) || (isset($attributes['displayFilter']) && $attributes['displayFilter']) ) {
                    ...
                    $markup_before .= '<div class="gpl-post-heading-wrapper gpl-post-heading-' . esc_attr($attributes['postHeadingAlign']) . ' gpl-post-heading-' . esc_attr($attributes['postHeadingStyle']) . ' ' . $has_filter . ' ">';
                    ...
                    if (isset($attributes['displayFilter']) && $attributes['displayFilter']) {
                        $markup_before .= '<div class="gpl-post-filter" data-filtertype="' . esc_attr($attributes['filterBy']) . '" data-postid="' . get_the_ID() . '" data-blockname="' . $blockName . '">';
                ...
                $markup_before .= esc_attr($attributes['gridLayoutStyle']) === 'g_skin2' ? '<div class="gpl-all-posts gpl-d-flex gpl-flex-wrap gpl-column-12">' : '';
                ...
                if ( $firstPostItem ) {
                    ...
                    $item_markup .= sprintf( '<article class="%1$s %2$s">', esc_attr( $gridView ), esc_attr($attributes['gridLayoutStyle']) );

                    // start the post content wrap
                    $item_markup .= '<div class="post-content-area ' . esc_attr($attributes['align']) . '">';
                ...
                if( isset($attributes['displayPagination']) && $attributes['displayPagination'] && isset($attributes['paginationType']) && $attributes['paginationType'] === 'gpl-pagination' ) {
                    $pagiAlign = isset($attributes['paginationAlign']) ? esc_attr($attributes['paginationAlign']) : '';
                ...
                if( isset($attributes['displayPagination']) && $attributes['displayPagination'] && isset($attributes['paginationType']) && $attributes['paginationType'] === 'gpl-navigation' ) {
                    ...
                    $pagiAlign = isset($attributes['paginationAlign']) && $navigationPosition != 'gpl-nav-top-right' ? esc_attr($attributes['paginationAlign']) : '';
				...
				<a class="gpl-cta-btn %1$s" href="%2$s" target="%3$s" rel="bookmark">%4$s %5$s</a>
						...
						esc_attr($attributes['CtaLinkTarget']),
                        esc_html( $attributes['postCtaButtonText'] ),
                        $icon,
                        esc_attr($attributes['postCtaButtonAlign']),
                        $widthClass
                    );
                }
            ...
            if ( is_preview() ) {
				if ( ( 'draft' !== $post->post_status ) && isset( $_GET['preview_id'], $_GET['preview_nonce'] ) ) {
					...
					if ( wp_verify_nonce( $preview_nonce, 'post_preview_'.$preview_id ) ) {
						$query_args['preview_id']    = $preview_id;
						$query_args['preview_nonce'] = $preview_nonce;
					}
				}
        ...
        public function filter( $filterBy = '', $displayAllButton = false, $allButtonText = '', $filterCats = '[]', $filterTags = '[]', $maxTaxDisplay = 5 ) {
            ...
            if( !empty($filterTags) || !empty($filterCats) ) {
                ...
                if ($displayAllButton && $allButtonText) {
                    $html .= '<li><a data-taxonomy="" href="#">' . esc_html($allButtonText) . '</a></li>';
                }
